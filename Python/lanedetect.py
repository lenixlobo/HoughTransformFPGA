...
def plot_line(img_lane_output, rho, theta, test, HT_offset_h, color):
	#generate lines from rho and theta
	a = np.cos(theta)
	b = np.sin(theta)
	x0 = a*rho
	y0 = b*rho + HT_offset_h
	x1 = int(x0 + 1000*(-b))
	y1 = int(y0 + 1000*(a))
	x2 = int(x0 - 1000*(-b))
	y2 = int(y0 - 1000*(a))
	cv2.line(img_lane_output, (x1,y1),(x2,y2),color,2)
...
for current_line in lines_left:
    ...
    for rho,theta in current_line:
        ...
        plot_line(img_lane_output, rho, theta, 0, HT_offset_h, (0, 0, 255))
...
for current_line in lines_right:
    ...
    for rho,theta in current_line:
        ...
        plot_line(img_lane_output, rho, theta, 0,HT_offset_h, (255, 0, 0)) 
